...
void BuildStatus::BuildEdgeStarted(const Edge* edge, const string &prefix) {
...
void BuildStatus::PrintStatus(const Edge* edge, EdgeStatus status, const string &prefix) {
...
void Plan::EdgeWanted(const Edge* edge) {
  ...
  if (edge->is_remote_) {
    ++remote_edges_;
    remote_rules_.insert(edge->rule_);
  }
}
...
Builder::Builder(IRemoteExecutor * const remoteExecutor, State* state, const BuildConfig& config,
    ...
    : state_(state), config_(config),
      plan_(this), remote_runner_(remoteExecutor), disk_interface_(disk_interface),
...
bool Builder::Build(string* err) {
  ...
  std::set<std::string> toolIdsSet;
  for (const Rule * rule : plan_.remote_rules())
  {
      if (!rule->toolId_.empty())
